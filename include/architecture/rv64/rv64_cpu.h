...
public:
    ...
    // quando rodado com mhartid() -1 da problema 
    ...
    static T finc(volatile T & value) {
        ...
        register T one = 1;
        if(sizeof(T) == sizeof(Reg64))
            ASM("amoadd.d %0, %1, (%2)" : "=&r"(old), "+r"(one) : "r"(&value) : "memory");
        else
            ASM("amoadd.w %0, %1, (%2)" : "=&r"(old), "+r"(one) : "r"(&value) : "memory");
        return old;
    }
